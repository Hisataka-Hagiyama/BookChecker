...
const FinishedList = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8080/books/finished/get')
            .then(res => {
    ...
    const deleteData = (index: number) => {
        if (window.confirm('削除します。よろしいですか？')) {
            axios.delete(`http://localhost:8080/books/finished/delete/${index}`)
            .then(()=>{
                    ...
                    <TableBody>
                        {listItems?.map((listItem) => (
                            ...
                            >
                                ...
                                <TableCell align="center"><Button endIcon={<DeleteIcon />} variant='contained' onClick={() => deleteData(listItem.id)}>削除</Button></TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
