...
const WishList = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:8080/books/wish/get')
            .then(res => {
    ...
    const deleteData = (index: number) => {
        if (window.confirm('削除します。よろしいですか？')) {
            axios.delete(`http://localhost:8080/books/wish/delete/${index}`)
            .then(()=>{
            axios.get('http://localhost:8080/books/wish/get')
                .then(res => 
                    setListItems(res.data)
    ...
    const moveToUnreadList=(index:number)=>{
        if(window.confirm("積読中に移動します。よろしいですか？")){
            axios.post(`http://localhost:8080/books/wish/move/${index}`)
            .then(()=>{
            axios.get('http://localhost:8080/books/wish/get')
                .then(res => 
                    setListItems(res.data)
                    ...
                    <TableBody>
                        {listItems?.map((listItem) => (
                            ...
                            >
                                ...
                                <TableCell align="center"><Button endIcon={<SendIcon />} variant='contained' onClick={() => moveToUnreadList(listItem.id)}>積読中へ</Button></TableCell>
